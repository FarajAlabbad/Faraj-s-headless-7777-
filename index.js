(async () => {
  ...
  const builds = {
    ...
    octo: "0/5/0/7/7/7/9/5/1",
    octos: "0/5/0/7/7/7/9/5/1",
    P4: "0/4/5/7/7/7/7/4/1/0"
  };
  ...
  const arras = (function () {
    ...
    const getStatus = function (f, s) {
            ...
            return {
              then: function (f) {
                try {
                  ...
                  log("Error parsing cached status JSON");
                }
              }
            }
          }
      ...
      realFetch(f).then(x => x.text()).then(x => {
        ...
        try {
          ...
          log("Server returned non-JSON status. Server might be down.");
        }
      }).catch(err => {
        log("Fetch error in getStatus:", err);
      });
      return {
        then: function () {
          return {
    ...
    const run = function (x, config, oa) {
          ...
          fillText: function () {
            ...
            if (this.font === 'bold 7px Ubuntu' && this.fillStyle === 'rgb(255,255,255)') {
              if (a[0] === `You have spawned! Welcome to the game.`) {
                ...
                if (process.send) process.send({ type: 'bot_spawned' });
              ...
              if (!died && (
                ...
                died = true
                if (process.send) process.send({ type: 'bot_died' });
              }
              if (!a[0].startsWith(`You're using an ad blocker.`) && a[0] !== 'Respawn' && a[0] !== 'Back' && a[0] !== 'Reconnect' && a[0].length > 2) {
                ...
                // --- KILL LOG FEED ---
                if (a[0].startsWith('You killed ') || a[0].startsWith('You assisted ')) {
                  if (process.send) process.send({ type: 'bot_kill', text: a[0] });
                }
              }
            }
              ...
              if (a[0].startsWith('Coordinates: (')) {
                ...
                if (b[l - 1] === ')') {
                  ...
                  if (b.length === 2) {
                    ...
                    // --- NETWORK AURA COORDS ---
                    if (process.send) process.send({ type: 'bot_coords', x: x, y: y });
                  }
                }
              }
            }
      ...
      // PATH FIND FUNC
      // Precise angle translation for direct multi loops
      ...
      // Safe release handler prevents ghost execution states
      function stopMoving() {
      ...
      const mainInterval = setInterval(function () {
        ...
        if (a) {
          ...
          if (inGame && config.type === 'follow') {
            ...
            if (target.x) {
              // Replicate the formation and target coordinates mathematically:
              let t = {
                x: target.x,
                y: target.y
              };

              // 1. Mouse Follow / Formation calculations
              if (target.isMouseFollowMode && target.mouseX !== undefined && target.mouseY !== undefined) {
                const hFov = target.fov || 46;
                const vFov = hFov * ((target.screenHeight || innerHeight) / (target.screenWidth || innerWidth));
                t.x = target.x + (target.mouseX - 0.5) * hFov;
                t.y = target.y + (target.mouseY - 0.5) * vFov;
              } else if (target.formationConfig && target.formationConfig.enabled && target.swarmSize > 1) {
                let index = target.swarmIndex === undefined ? 0 : target.swarmIndex;
                let size = target.swarmSize || 1;
                let spacingUnit = (target.formationConfig.spacing || 60) / 10.0;
                let aimUx = 1, aimUy = 0;

                if (target.mouseX !== undefined && target.mouseY !== undefined) {
                  let ax = target.mouseX - 0.5;
                  let ay = target.mouseY - 0.5;
                ...
                const type = target.formationConfig.type;

                if (type === 'line_front') {
                  offsetX = aimUx * (spacingUnit * index);
                  offsetY = aimUy * (spacingUnit * index);
                } else if (type === 'line_behind') {
                  offsetX = -aimUx * (spacingUnit * index);
                  offsetY = -aimUy * (spacingUnit * index);
                } else if (type === 'line_top') {
                  offsetX = aimUy * (spacingUnit * index);
                  offsetY = -aimUx * (spacingUnit * index);
                } else if (type === 'line_bottom') {
                  offsetX = -aimUy * (spacingUnit * index);
                  offsetY = aimUx * (spacingUnit * index);
                } else if (type === 'circle') {
                  const r = spacingUnit + (size * 0.5);
                  const ang = (Math.PI * 2 / size) * index;
                  offsetX = Math.cos(ang) * r;
                  offsetY = Math.sin(ang) * r;
                } else if (type === 'heart') {
                  // Heart formation
                  let tVal = -Math.PI + (Math.PI * 2 / size) * index;
                  let hX = 16 * Math.pow(Math.sin(tVal), 3);
                  let hY = -(13 * Math.cos(tVal) - 5 * Math.cos(2*tVal) - 2 * Math.cos(3*tVal) - Math.cos(4*tVal));
                  offsetX = (hX / 16) * spacingUnit * 2.5;
                  offsetY = (hY / 16) * spacingUnit * 2.5;
                  let rotAng = Math.atan2(aimUy, aimUx) - Math.PI/2;
                  let rx = offsetX * Math.cos(rotAng) - offsetY * Math.sin(rotAng);
                  let ry = offsetX * Math.sin(rotAng) + offsetY * Math.cos(rotAng);
                  offsetX = rx;
                  offsetY = ry;
                  ...
                  let perpUx = -aimUy;
                  let perpUy = aimUx;
                  offsetX = -aimUx * (spacingUnit * row) + perpUx * (spacingUnit * row * side);
                  offsetY = -aimUy * (spacingUnit * row) + perpUy * (spacingUnit * row * side);
                } else if (type === 'wall') {
                  // Perpendicular shield wall
                  let half = Math.floor(size / 2);
                  let posIndex = index - half;
                  let perpUx = -aimUy;
                  let perpUy = aimUx;
                  let row = index % 2 === 0 ? 0 : 0.5;
                  offsetX = -aimUx * (spacingUnit * row) + perpUx * (spacingUnit * posIndex);
                  offsetY = -aimUy * (spacingUnit * row) + perpUy * (spacingUnit * posIndex);
                } else if (type === 'wedge') {
                  // Delta wedge shape pointing forward
                  let side = index % 2 === 0 ? 1 : -1;
                  let row = Math.ceil(index / 2);
                  let perpUx = -aimUy;
                  let perpUy = aimUx;
                  offsetX = aimUx * (spacingUnit * row) + perpUx * (spacingUnit * row * side * 0.6);
                  offsetY = aimUy * (spacingUnit * row) + perpUy * (spacingUnit * row * side * 0.6);
                } else if (type === 'orbit') {
                  let orbitSpeed = target.formationConfig.orbitSpeed || 10;
                  let r = spacingUnit + (size * 0.5);
                  let timeOffset = (Date.now() / (orbitSpeed * 1000)) * Math.PI * 2;
                  let ang = (Math.PI * 2 / size) * index + timeOffset;
                  offsetX = Math.cos(ang) * r;
                  offsetY = Math.sin(ang) * r;
                } else if (type === 'pincer') {
                  let side = index % 2 === 0 ? 1 : -1;
                  ...
                  let perpUx = -aimUy;
                  let perpUy = aimUx;
                  if (target.formationConfig.pincerActive) {
                    offsetX = aimUx * (spacingUnit * row * 1.5) + perpUx * (spacingUnit * row * side * 0.3);
                    offsetY = aimUy * (spacingUnit * row * 1.5) + perpUy * (spacingUnit * row * side * 0.3);
                  } else {
                    offsetX = -aimUx * (spacingUnit * row) + perpUx * (spacingUnit * row * side * 1.2);
                    offsetY = -aimUy * (spacingUnit * row) + perpUy * (spacingUnit * row * side * 1.2);
                  }
                } else if (type === 'square') {
                  let cols = Math.ceil(Math.sqrt(size));
                  let row = Math.floor(index / cols);
                  let col = index % cols;
                  let centerRow = (Math.ceil(size / cols) - 1) / 2;
                  let centerCol = (cols - 1) / 2;
                  let gridX = (col - centerCol) * spacingUnit;
                  let gridY = (row - centerRow) * spacingUnit;
                  let rotAng = Math.atan2(aimUy, aimUx) - Math.PI/2;
                  offsetX = gridX * Math.cos(rotAng) - gridY * Math.sin(rotAng);
                  offsetY = gridX * Math.sin(rotAng) + gridY * Math.cos(rotAng);
                } else if (type === 'text') {
                  if (target.formationConfig.customPixels && target.formationConfig.customPixels.length > 0) {
                    let pixels = target.formationConfig.customPixels;
                    let pixel = pixels[index % pixels.length];
                    
                    let gridX = pixel.x * (spacingUnit * 0.5);
                    let gridY = pixel.y * (spacingUnit * 0.5);
                ...
                // Starburst Scatter Override
                if (target.formationConfig && target.formationConfig.isScattering) {
                    let ang = (Math.PI * 2 / size) * index;
                    t.x = position[0] + Math.cos(ang) * 1000;
                    t.y = position[1] + Math.sin(ang) * 1000;
                }
              } else if (target.followMouse) {
                // Backward compatible legacy look
                t.x += target.mouseX;
                t.y += target.mouseY;
              }

              // 2. Movement executing with Stop Distance Threshold
              let dist = Math.hypot(t.x - position[0], t.y - position[1]);

              if (target.movementEnabled !== false) {
                  let stopThreshold = target.preciseMovement ? 1.0 : 0.1;
                  if (!target.isMouseFollowMode && !(target.formationConfig && target.formationConfig.enabled)) {
                      stopThreshold = Math.max(stopThreshold, (target.followDistance || 0) / 10.0);
                  }
              ...
              // 3. Aim Angle Calculation
              let angle;
              if (target.shift) {
                angle = Math.atan2(target.mouseY, target.mouseX);
              } else if (target.isAimReplication !== false && target.mouseX !== undefined && target.mouseY !== undefined) {
                if (target.isPreciseAim !== false) {
                  const lW = target.screenWidth || innerWidth;
                  const lH = target.screenHeight || innerHeight;
                  const lFov = target.fov || 46;
                  const lVFov = lFov * (lH / lW);
                  const targetWorldX = target.x + (target.mouseX - 0.5) * lFov;
                  const targetWorldY = target.y + (target.mouseY - 0.5) * lVFov;
                  angle = getDir(position[0], position[1], targetWorldX, targetWorldY);
                } else {
                  // Simple aiming vector copy (non-precise)
                  angle = Math.atan2(target.mouseY - 0.5, target.mouseX - 0.5);
                }
              } else {
                angle = getDir(
                  position[0],
          ...
          if (url.endsWith('/status')) {
            fetchPromise.then(async (response) => {
              ...
              if (contentType && contentType.includes("application/json")) {
                try {
                  const i = await response.clone().json();
                  if (i.ok && i.status) {
                    statusRecieved = true;
                  ...
                  log(`JSON parse error for status (${url}):`, e);
                }
              } else {
                log(`Expected JSON but got ${contentType || 'unknown'} for status (${url}). Server might be down or blocking bots.`);
              }
            }).catch(err => {
  ...
  process.on('message', (message) => {
    if (message.type === 'start') {
      ...
      // --- MOVEMENT TOGGLE ---
      target.movementEnabled = message.movementEnabled;

      // --- EXPANDED SYNC PARAMETERS ---
      target.isAimReplication = message.isAimReplication;
      target.isPreciseAim = message.isPreciseAim;
      target.isMouseFollowMode = message.isMouseFollowMode;
      target.followDistance = message.followDistance;
      target.formationConfig = message.formationConfig;
      target.preciseMovement = message.preciseMovement;
      
      target.screenWidth = message.screenWidth;
      target.screenHeight = message.screenHeight;
      target.fov = message.fov;
      
      target.swarmIndex = message.swarmIndex;
      target.swarmSize = message.swarmSize;
      ...
      // --- GLOBAL CHAT MIRROR ---
      if (currentBotInterface.controller) {
        currentBotInterface.controller.chat(message.msg);
      }
    } else if (message.type == 'key_states') {
      // --- KEY REPLICATION ---
      if (currentBotInterface.trigger) {
        for (const [key, state] of Object.entries(message.keys)) {
          if (state) {
            currentBotInterface.trigger.keydown(key);
          } else {
            currentBotInterface.trigger.keyup(key);
          }
        }
      }
    }
